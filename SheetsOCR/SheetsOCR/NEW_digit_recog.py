...
class SVM_MODEL():
    def __init__(self, num_feats, C = 1, gamma = 0.1):
        ...
        self.model.setKernel(cv2.ml.SVM_RBF) #SVM_LINEAR, SVM_RBF radial_basis_function(when value is between 0 and 1, similarity is detected)
        self.model.setC(C)  #It is a parameter that generates an influence over the model
        self.model.setGamma(gamma) #if calculated gamma is large then bias generated is high
...
digits, labels = load_digits_custom("custom_train_digits.jpg") #my handwritten dataset (better than MNIST on my handwritten digits)
